...
public class Tour_list extends AppCompatActivity {
        ...
        recyclerView.addOnItemTouchListener(new RecyclerView.OnItemTouchListener() {
            ...
            public boolean onInterceptTouchEvent(@NonNull RecyclerView rv, @NonNull MotionEvent e) {
                ...
                if(child != null && gestureDetector.onTouchEvent(e)){
                    TextView tv = recyclerView.getChildViewHolder(child).itemView.findViewById(R.id.tno);
                    tour_picture = Integer.parseInt(tv.getText().toString());
                    link = tourList.get(tour_picture).getLink();
    ...
    private void prepareTourData(){
        Tour tour = new Tour(R.drawable.t_0,"에펠탑","프랑스",
                ...
                "격자 구조로 이루어져 파리에서 가장 높은 건축물이며, 매년 수백만 명이 방문할 만큼 세계적인 유료 관람지이다.",0);
        tourList.add(tour);

        tour = new Tour(R.drawable.t_1,"타지마할","인도",
                "http://getabout.hanatour.com/archives/135887",
                "타지마할은 세계 불가사의 건축물이고 애틋한 설화가 존재하는 매우 아름다운 건축물이다.",1);
        tourList.add(tour);
        ...
        tour = new Tour(R.drawable.t_5,"스톤헨지","영국",
                ...
                "고대시대 건설된 스톤헨지는 죽은 사람을 위한 종교적 공간인 신성한 장소였다.",5);
        tourList.add(tour);

        tour = new Tour(R.drawable.t_6,"알함브라 궁전","스페인",
                "http://blog.eztok.co.kr/trilingual/?p=723",
                "극도로 세련된 아름다움이 있어 높이 평가받는 건물이며 그리스도교 세계와 이슬람 세계의 건축이 절충이 된 건축물이다.",6);
        tourList.add(tour);
        ...
        tour = new Tour(R.drawable.t_7,"페트라","요르단",
                ...
                "바위를 깎아 만든, 암벽에 세워진 도시로 페트라라는 뜻은 바위를 뜻한다. 아름다운 산악 도시이다.",7);
        tourList.add(tour);
        ...
        tour = new Tour(R.drawable.t_12,"자유의 여신상","미국",
                ...
                "미국의 독립 100주년을 기념하기 위해 프랑스 국민들의 모금 운동으로 증정되었으며, 1886년에 완공되었다.",12);
        tourList.add(tour);

        tour = new Tour(R.drawable.t_13,"노이슈반슈타인 성","독일",
                "https://calisol.tistory.com/241",
                "루트비히 2세가 바그너와 그의 오페라인 로엔그린에 푹 빠진 탓에 취미로 지은 성이다.",13);
        tourList.add(tour);

        tour = new Tour(R.drawable.t_14,"세인트 바 실의 성당","러시아",
                "https://toytvstory.tistory.com/919",
                "모스크바 대공국의 대공이었던 이반 4세가 러시아에서 카잔 칸을 몰아낸 것을 기념하며 봉헌한 성당이다.",14);
        tourList.add(tour);
